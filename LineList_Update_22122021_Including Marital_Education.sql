...
f.`family_name` Surname, f.`given_name` FirstName, g.`patient_id` AS Biometrics, h.Occupation, 
i.Civil_status, j.Highest_Educational_Level
FROM `patient` a 
...
/*===============================================================================================*/
LEFT JOIN (SELECT e.`patient_id`, ConceptName (`value_coded`) Occupation
FROM `encounter` e LEFT JOIN obs o ON e.`patient_id`=o.`person_id` WHERE e.`encounter_type`=14 
AND o.`concept_id`=1542 AND o.`voided`=0) h ON a.`patient_id` = h.`patient_id`

LEFT JOIN (SELECT e.`patient_id`, ConceptName (`value_coded`) Civil_status
FROM `encounter` e LEFT JOIN obs o ON e.`patient_id`=o.`person_id` WHERE e.`encounter_type`=14 
AND o.`concept_id`=1054 AND o.`voided`=0) i ON a.`patient_id` = i.`patient_id`

LEFT JOIN (SELECT e.`patient_id`, ConceptName (`value_coded`) Highest_Educational_Level
FROM `encounter` e LEFT JOIN obs o ON e.`patient_id`=o.`person_id` WHERE e.`encounter_type`=14 
AND o.`concept_id`=1712 AND o.`voided`=0) j ON a.`patient_id` = j.`patient_id`
/*===============================================================================================*/
...
a.Phone_No, CONCAT_WS(', ', a.`address2`,  a.`address1`, a.`city_village`, a.`state_province`)Address, a.Occupation, 
a.Civil_status, a.Highest_Educational_Level,
