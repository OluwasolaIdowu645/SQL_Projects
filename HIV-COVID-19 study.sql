...
SELECT o.`person_id`, o.`value_numeric` AS ARTStartDate
FROM `obs` o LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` 
...
SELECT o.`person_id`, o.`concept_id`, o.`obs_datetime`, get_concept_name(o.`value_coded`) Regimen  FROM `obs` o 
...
SELECT @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, a.`patient_id`, a.`identifier` PEPID, CAST(DATEDIFF(CURDATE(), b1.birthdate)/(SELECT DAYOFYEAR(CONCAT(YEAR(CURDATE()),'-12-31')))  AS UNSIGNED) AS Age,
b1.gender SEX,  c.Marrital 'Marital status', b.Education 'Educational level', d.Occu_Status 'Occupation',
h.`city_village` 'LGA of residence', h.`state_province` 'State' , e.DateConfirmed 'Date of diagnosis ', i.encounter_datetime 'Date of enrolment into care', j.ARTStartDate 'Date of ART initiation',
k.ARTStartDate AS'CD4 result at ART initiation', l.1st_CD4 '1st_CD4_Recorded(Lab Form)', 1st_CD4_Date Date_1st_CD4,  Last_CD4 'Last_CD4(Lab Form)', Last_CD4_Date, n.Regimen 'ART regimen at Start of ART',
...
'' AS 'Current ART regimen type', '' AS 'Date of last drug pick up', '' AS 'Outcome variables', '' AS 'Date of last drug pick up', '' AS 'Quantity of drugs (in days) dispensed at last drug pick-up scheduled appointment',
'' AS 'Date of last appointment ', '' AS 'Last VL result known'
