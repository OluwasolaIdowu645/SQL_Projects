...
SELECT a.`patient_Id`, a.`date_created` Date_Captured, IF(b.`patient_Id` IS NOT NULL, 'Low Quality', NULL) PBS_Quality FROM `biometricinfo` a
LEFT JOIN (SELECT `patient_Id` FROM `biometricinfo` WHERE `imageQuality` < 60 GROUP BY patient_Id) b ON a.`patient_Id`=b.`patient_Id`
GROUP BY `patient_Id`;

/*---Next of Kin Table------------------------------------------------------------------------------------------------------------------------------------------------*/
DROP TABLE IF EXISTS Next_of_Kin;
CREATE TEMPORARY TABLE Next_of_Kin AS
...
f.`family_name` Surname, f.`given_name` FirstName, g.`patient_id` AS Biometrics, g.Date_Captured, g.PBS_Quality,
h.Next_of_kin_Name, h.Next_Of_Kin_Relationship, h.Next_of_kin_Phone_No, h.KP_Type
FROM `patient` a 
...
/*------------------------------------------------------------------------------------------------------------------------------*/
LEFT JOIN (SELECT * FROM Next_of_Kin WHERE Next_of_kin_Name IS NOT NULL AND Next_Of_Kin_Relationship IS NOT NULL 
AND Next_of_kin_Phone_No IS NOT NULL AND KP_Type IS NOT NULL) h ON a.`patient_id`=h.`patient_id`
/*-------------------------------------------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
DROP TABLE IF EXISTS CIHP_Med_Strength;
CREATE TEMPORARY TABLE CIHP_Med_Strength AS
SELECT o.`person_id`, e.`encounter_id`, e.`encounter_datetime` LastPickupDate, o.`obs_group_id`, ConceptName(o.`value_coded`) Drug_Strength, a.`obs_id` FROM `obs` o 
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id` AND  o.`encounter_id` = e.`encounter_id`
LEFT JOIN (SELECT o2.`obs_id` FROM `obs` o2 WHERE o2.concept_id = 162240 AND o2.`voided` = 0) a ON a.obs_id = o.obs_group_id
WHERE  e.Occ = 1 AND o.`concept_id` = 165725 AND o.`voided` = 0;
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
SELECT a.`person_id`, f.ARTStartDate, a.LastPickupDate, a.DaysOfARVRefill, a.Pill_Balance, a.Next_Pickup_Date, e.RegLineAtStart, b.ARTRegAtStart, 
d.CurrentRegLine, c.CurrentARTReg, n.Drug_Strength, g.PregStatus, h.CurrentVL, h.DateOfCurrentVL, h.Date_Result_Received, i.ViralLoadIndication, m.Last_VL_Sample_Date, j.TI, 
...
LEFT JOIN CIHP_Med_Strength n ON a.person_id=n.person_id;
...
(SELECT @row_no := IF(@prev_val = e.`patient_id` , @row_no + 1, 1) AS Occurrence, @prev_val := e.`patient_id` AS patient_id,
 e.`encounter_id`, e.`encounter_datetime`
  ...
  AND e.`form_id` = 53 AND e.`voided`= 0 AND e.`encounter_datetime` <= @Reporting_Date
...
a.Pepid, b.HospitalNo, IF((a.sex = 'F'), 'Female', 'Male') Sex,  a.KP_Type, a.KP_Type,
CAST(DATEDIFF(c.ARTStartDate, a.`DOB`)/(SELECT DAYOFYEAR(CONCAT(YEAR(@Reporting_Date),'-12-31')))  AS UNSIGNED) AS AgeAtStartofART,
CAST(DATEDIFF(c.ARTStartDate, a.`DOB`)/((SELECT DAYOFYEAR(CONCAT(YEAR(@Reporting_Date),'-12-31')))/12)  AS UNSIGNED) AS AgeAtStart_InMonth,
c.ARTStartDate, c.LastPickupDate, c.DaysOfARVRefill, c.RegLineAtStart, c.ARTRegAtStart, 
c.CurrentRegLine, c.CurrentARTReg, c.Drug_Strength, c.PregStatus, c.CurrentVL, c.DateOfCurrentVL, c.ViralLoadIndication, c.Date_Result_Received, c.Last_VL_Sample_Date,
...
c.TI, a.DOB, CAST(DATEDIFF(@Reporting_Date, a.`DOB`)/(SELECT DAYOFYEAR(CONCAT(YEAR(@Reporting_Date),'-12-31'))) AS UNSIGNED) AS CurrentAge,a.Surname, a.FirstName,
a.Phone_No, CONCAT_WS(', ', a.`address2`,  a.`address1`, a.`city_village`, a.`state_province`)Address, a.Next_of_kin_Name, a.Next_Of_Kin_Relationship, a.Next_of_kin_Phone_No,
c.NextAppmt, c.Pill_Balance, c.Next_Pickup_Date, @DATIMCode DATIMCode, ROUND((TO_DAYS(@Reporting_Date) - TO_DAYS(C.ARTStartDate)) / ((SELECT DAYOFYEAR(CONCAT(YEAR(@Reporting_Date),'-12-31')))/12)) AS MonthOnART,
c.Weight Last_Weight, c.Visit_Date Last_Weight_Date, c.First_TLD_Date, IF(a.Biometrics IS NOT NULL, "Yes", "") AS Biometrics_Captured, a.Date_Captured, a.PBS_Quality,
e.Eligible_For_IPT, e.Date_IPT_start, e.Outcome_of_IPT, e.Date_of_Outcome, j.Last_Clinic_Visit_Date, g.Last_TB_Screening_Date, g.Last_TB_Screening_Status,
 e2.TB_Investigations, e2.Investig_Result, e2.Date_TB_Investig,
...
LEFT JOIN All_IPT e ON a.patient_id=e.patient_id 
LEFT JOIN Last_All_TB_Investig e2 ON a.patient_id=e2.patient_id 
...
LEFT JOIN (SELECT e.`patient_id`, MAX(e.`encounter_datetime`) Last_Clinic_Visit_Date FROM `encounter` e
 WHERE e.`encounter_type` = 12 AND e.`form_id` = 14  AND e.`voided` = 0 AND e.`encounter_datetime` <= @Reporting_Date GROUP BY e.patient_id) j
