...
DROP PROCEDURE IF EXISTS `CIHP_LineList_31st_Dec`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `CIHP_LineList_31st_Dec`()
BEGIN
SET @ValidVLDate1 := '2021-12-31' - INTERVAL (SELECT DAYOFYEAR(CONCAT(YEAR('2021-12-31'),'-12-31'))) DAY;
...
WHERE en.`encounter_type` = 14 AND en.`encounter_datetime` <= '2021-12-31' AND en.`voided`=0 
...
WHERE o.`concept_id` = 159599 AND e.`encounter_type` = 25 AND e.`form_id` = 56 AND o.`voided`=0 AND o.`value_datetime` <= '2021-12-31';
...
WHERE e.`encounter_type` = 13 AND e.`form_id` = 27 AND e.`voided` = 0 AND e.`encounter_datetime` <= '2021-12-31' 
...
AND o2.`obs_datetime` <= '2021-12-31') GROUP BY o.`person_id`, o.`obs_datetime`;
SET SESSION sql_mode = '';
/*CIHP_PregStatus*/
...
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id` = 856 AND e.`encounter_type` = 11 AND e.`form_id` = 21  AND o.`voided` = 0 AND e.`encounter_datetime` <= '2021-12-31'
ORDER BY e.patient_id, e.`encounter_datetime` DESC;
DROP TABLE IF EXISTS VL_Search;
CREATE TEMPORARY TABLE VL_Search AS
SELECT * FROM CIHP_ViralLoad_Data WHERE Occurrence = 1 AND DateOfCurrentVL <= '2021-12-31';
/*CIHP_VL_Indication*/
...
LEFT JOIN `concept_name` c2 ON o.`value_coded`=c2.`concept_id` AND c2.locale = 'en' AND c2.locale_preferred = 1
LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
WHERE e.`encounter_type` = 14 AND e.`form_id` = 23 AND o.`concept_id` = 160540 AND o.`voided` = 0  AND o.`obs_datetime` <= '2021-12-31' AND c2.`concept_id`=160563
...
(SELECT MAX(o2.obs_datetime) FROM obs o2 WHERE o.person_id=o2.person_id AND o2.concept_id = 5089 and o2.obs_datetime <= '2021-12-31') GROUP BY o.person_id;*/
...
LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4 AND p2.`preferred` = 1,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.concept_id = 5089 AND p2.`identifier_type`=4  AND o.`voided` = 0 AND o.`obs_datetime` <= '2021-12-31'
ORDER BY o.person_id, o.`obs_datetime` DESC;
DROP TABLE IF EXISTS Last_Weight;
CREATE TEMPORARY TABLE Last_Weight AS
SELECT * FROM All_Weight WHERE Occurrence = 1 AND Visit_Date <= '2021-12-31';
SET SESSION sql_mode = '';
...
WHERE o.concept_id = 159951 AND e.`form_id`=67 AND p.`identifier_type`=4 AND o.`value_datetime` BETWEEN @ValidVLDate1 AND '2021-12-31';*/
...
LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4 AND p2.`preferred` = 1,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.concept_id = 159951 AND e.`form_id`=67 AND p2.`identifier_type`=4  AND o.`voided` = 0 AND o.`obs_datetime` <= '2021-12-31'
ORDER BY o.person_id, o.`obs_datetime` DESC;
DROP TABLE IF EXISTS CIHP_VL_SampleCol;
CREATE TEMPORARY TABLE CIHP_VL_SampleCol AS
SELECT * FROM CIHP_All_VL_SampleCol WHERE Occurrence = 1 AND Sample_Date <= '2021-12-31';*/
...
LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE concept_id = 159951 AND o.voided=0 AND e.`encounter_type` = 11 AND e.`form_id` = 21 AND o.`obs_datetime` <= '2021-12-31'
ORDER BY o.person_id, o.`obs_datetime` DESC;
DROP TABLE IF EXISTS CIHP_VL_SampleCol;
CREATE TEMPORARY TABLE CIHP_VL_SampleCol AS
...
CREATE TEMPORARY TABLE All_IPT AS
  ...
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE e.`encounter_type` = 23 AND e.`form_id` = 53 AND o.`voided`= 0 AND o.`obs_datetime` <= '2021-12-31'
  GROUP BY person_id;
...
LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4 AND p2.`preferred` = 1,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id` IN (166141, 166142, 166143, 166144)  AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided` = 0 AND o.`obs_datetime` <= '2021-12-31'
ORDER BY o.person_id, o.`obs_datetime` DESC; 
...
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id`=166097 AND e.`encounter_type`=32 AND e.`encounter_datetime`<= '2021-12-31' ORDER BY o.person_id, o.`obs_datetime` DESC;
...
ON a.`person_id`=o.`person_id` AND a.`encounter_id`=e.`encounter_id`,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id`=166097 AND e.`encounter_type`=32 AND e.`encounter_datetime`<= '2021-12-31' ORDER BY o.person_id, o.`obs_datetime` ASC;
...
CAST(DATEDIFF(c.ARTStartDate, a.`DOB`)/(SELECT DAYOFYEAR(CONCAT(YEAR('2021-12-31'),'-12-31')))  AS UNSIGNED) AS AgeAtStartofART,
CAST(DATEDIFF(c.ARTStartDate, a.`DOB`)/((SELECT DAYOFYEAR(CONCAT(YEAR('2021-12-31'),'-12-31')))/12)  AS UNSIGNED) AS AgeAtStart_InMonth,
...
CASE 
WHEN (FLOOR((TO_DAYS('2021-12-31') - TO_DAYS(c.NextAppmt))) <= 28) AND (d.Date_of_Termination IS NULL OR d.Date_of_Termination='')
THEN 'Active'
...
WHEN (a.`death_date` <= '2021-12-31' AND a.`dead` = 1)
THEN 'Dead'
ELSE 'In-Active'
END AS CurrentARTStatus_28Days,
CASE WHEN (FLOOR((TO_DAYS('2021-12-31') - TO_DAYS(c.NextAppmt))) <= 90)
THEN 'Active'
ELSE 'In-Active'
END AS CurrentARTStatus_90Days,
FLOOR((TO_DAYS('2021-12-31') - TO_DAYS(c.NextAppmt))) AS DaysInterval,
c.TI, a.DOB, CAST(DATEDIFF('2021-12-31', a.`DOB`)/365.3 AS UNSIGNED) AS CurrentAge,a.Surname, a.FirstName,
a.Phone_No, CONCAT_WS(', ', a.`address2`,  a.`address1`, a.`city_village`, a.`state_province`)Address,
c.NextAppmt, @DATIMCode DATIMCode, ROUND((TO_DAYS('2021-12-31') - TO_DAYS(C.ARTStartDate)) / ((SELECT DAYOFYEAR(CONCAT(YEAR('2021-12-31'),'-12-31')))/12)) AS MonthOnART,
...
FROM `obs` o LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id` =1659  AND e.`encounter_type` = 12 AND e.`form_id` = 14  AND o.`voided` = 0 AND o.`obs_datetime` <= '2021-12-31'
...
FROM `obs` o LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id` =5497  AND e.`encounter_type` = 11 AND e.`form_id` = 21  AND o.`voided` = 0 AND o.`obs_datetime` <= '2021-12-31'
...
LEFT JOIN (SELECT e.`patient_id`, MAX(e.`encounter_datetime`) Last_Clinic_Visit_Date FROM `encounter` e
 WHERE e.`encounter_type` = 12 AND e.`form_id` = 14  AND e.`voided` = 0 AND e.`encounter_datetime` <= '2021-12-31' GROUP BY e.patient_id) j
