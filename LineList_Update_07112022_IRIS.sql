...
f.`family_name` Surname, f.`given_name` FirstName, g.`Date_Captured` AS Biometrics
FROM `patient` a 
...
g.Last_TB_Screening_Date, g.Last_TB_Screening_Status, e.TB_Investigations, e.Investig_Result, e.Date_TB_Investig,  d.`Reason for Tracking`, d.obs_datetime AS Tracking_Date,
...
AND d.Date_Returned IS NOT NULL, d.Date_Returned, d.Date_of_Termination)Date_of_Termination, f.Last_EAC_Session_Date,
  ...
  CONCAT(LEFT(UUID(), 4),RIGHT(YEAR(CURDATE()), 2),"-" , LEFT(UUID(), 5),MONTH(CURDATE()),"-", LEFT(UUID(), 3), DAY(CURDATE())) List_Parameters 
FROM `CIHP_Patient` a 
