...
SELECT * FROM (SELECT @row_no := IF(@prev_val = e.patient_id , @row_no + 1, 1) AS Occ, @prev_val := e.patient_id AS patient_id, e.`encounter_datetime`, e.`encounter_id`
...
ORDER BY e.patient_id, e.`encounter_datetime` DESC) a WHERE a.Occ = 1;
SET SESSION sql_mode = '';
...
SELECT o.`obs_id`, o.`person_id`, o.`concept_id`, e.`encounter_datetime`, o.`value_coded`, ConceptName(o.`value_coded`) CurrentARTReg FROM `obs` o 
...
SELECT o.`person_id`, o.`concept_id`, o.`obs_datetime`, o.`value_coded`, ConceptName(o.`value_coded`) CurrentRegLine  FROM `obs` o 
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id`
...
SELECT o.`person_id`, e.`encounter_id`, e.`encounter_datetime` LastPickupDate, o.`obs_group_id`, 
GROUP_CONCAT(DISTINCT ConceptName(o.`value_coded`) SEPARATOR '-') ARV_Drug_Strength, a.`obs_id` FROM `obs` o
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id` AND  o.`encounter_id` = e.`encounter_id`
LEFT JOIN (SELECT o2.`obs_id` FROM `obs` o2 WHERE o2.concept_id = 162240 AND o2.`voided` = 0) a ON a.obs_id = o.obs_group_id
WHERE  e.Occ = 1 AND o.`concept_id` = 165725 AND o.`obs_group_id`=a.`obs_id` AND o.`voided` = 0 GROUP BY o.`person_id`;
#OI Drug CTX
DROP TABLE IF EXISTS c1;CREATE TEMPORARY TABLE c1 AS
SELECT o.`person_id`, e.`encounter_id`, e.`encounter_datetime` LastPickupDate, o.`obs_group_id`, ConceptName(o.`value_coded`) OI_Drug_CTX,
a.`obs_id` FROM `obs` o 
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id` AND  o.`encounter_id` = e.`encounter_id`
LEFT JOIN (SELECT o2.`obs_id` FROM `obs` o2 WHERE o2.concept_id = 165726 AND o2.`voided` = 0) a ON a.obs_id = o.obs_group_id
WHERE  e.Occ = 1 AND o.`concept_id` = 165727 AND o.`obs_group_id`=a.`obs_id` AND o.`voided` = 0 AND ConceptName(o.`value_coded`) LIKE 'CTX%';
#OI Drug INH
DROP TABLE IF EXISTS d1;CREATE TEMPORARY TABLE d1 AS
SELECT o.`person_id`, e.`encounter_id`, e.`encounter_datetime` LastPickupDate, o.`obs_group_id`, ConceptName(o.`value_coded`) OI_Drug_INH,
a.`obs_id` FROM `obs` o 
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id` AND  o.`encounter_id` = e.`encounter_id`
LEFT JOIN (SELECT o2.`obs_id` FROM `obs` o2 WHERE o2.concept_id = 165726 AND o2.`voided` = 0) a ON a.obs_id = o.obs_group_id
WHERE  e.Occ = 1 AND o.`concept_id` = 165727 AND o.`obs_group_id`=a.`obs_id` AND o.`voided` = 0 AND ConceptName(o.`value_coded`) LIKE 'IS%';
#OI Drug Strength CTX
DROP TABLE IF EXISTS e1;CREATE TEMPORARY TABLE e1 AS
SELECT o.`person_id`, e.`encounter_id`, e.`encounter_datetime` LastPickupDate, o.`obs_group_id`,  ConceptName(o.`value_coded`) OI_Drug_Strength,
a.`obs_id` FROM `obs` o 
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id` AND  o.`encounter_id` = e.`encounter_id`
LEFT JOIN (SELECT o2.`obs_id` FROM `obs` o2 WHERE o2.concept_id = 165726 AND o2.`voided` = 0) a ON a.obs_id = o.obs_group_id
WHERE  e.Occ = 1 AND o.`concept_id` = 165725 AND o.`obs_group_id`=a.`obs_id` AND o.`voided` = 0;
#OI Drug Strength INH
DROP TABLE IF EXISTS f1;CREATE TEMPORARY TABLE f1 AS
SELECT o.`person_id`, e.`encounter_id`, e.`encounter_datetime` LastPickupDate, o.`obs_group_id`,  ConceptName(o.`value_coded`) OI_Drug_Strength,
a.`obs_id` FROM `obs` o 
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id` AND  o.`encounter_id` = e.`encounter_id`
LEFT JOIN (SELECT o2.`obs_id` FROM `obs` o2 WHERE o2.concept_id = 165726 AND o2.`voided` = 0) a ON a.obs_id = o.obs_group_id
...
DROP TABLE IF EXISTS g1;CREATE TEMPORARY TABLE g1 AS
SELECT o.`person_id`, e.`encounter_id`, e.`encounter_datetime` LastPickupDate, o.`obs_group_id`,  ConceptName(o.`value_coded`) DSD_Model
FROM `obs` o 
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id` AND  o.`encounter_id` = e.`encounter_id`
WHERE  e.Occ = 1 AND o.`concept_id` = 166148 AND o.`voided` = 0;
# Facility Dispensing (166276) or DDD
DROP TABLE IF EXISTS h1;CREATE TEMPORARY TABLE h1 AS
SELECT o.`person_id`, e.`encounter_id`, e.`encounter_datetime` LastPickupDate, o.`obs_group_id`,  ConceptName(o.`value_coded`) DDD_Fac_Disp
FROM `obs` o 
LEFT JOIN `CIHP_PharmOcc` e ON o.`encounter_id`=e.`encounter_id` AND  o.`encounter_id` = e.`encounter_id`
WHERE  e.Occ = 1 AND o.`concept_id` IN (166276, 166363) AND o.`voided` = 0;

DROP TABLE IF EXISTS CIHP_Med_duration;
CREATE TEMPORARY TABLE CIHP_Med_duration AS
SELECT a.*, b1.ARV_Drug_Strength, c1.OI_Drug_CTX, e1.OI_Drug_Strength CTX_Strength, d1.OI_Drug_INH, f1.OI_Drug_Strength INH_Strength, g1.DSD_Model, h1.DDD_Fac_Disp
FROM CIHP_Med_duratn a
LEFT JOIN b1 ON a.encounter_id=b1.encounter_id AND a.person_id=b1.person_id
LEFT JOIN c1 ON a.encounter_id=c1.encounter_id AND a.person_id=c1.person_id
LEFT JOIN d1 ON a.encounter_id=d1.encounter_id AND a.person_id=d1.person_id
LEFT JOIN e1 ON c1.obs_id=e1.obs_group_id AND a.person_id=e1.person_id
LEFT JOIN f1 ON d1.obs_id=f1.obs_group_id AND a.person_id=f1.person_id
LEFT JOIN g1 ON a.encounter_id=g1.encounter_id AND a.person_id=g1.person_id
LEFT JOIN h1 ON a.encounter_id=h1.encounter_id AND a.person_id=h1.person_id
WHERE a.obs_id IS NOT NULL GROUP BY person_id, encounter_id, LastPickupDate;
...
p.`gender`, o.`concept_id`, o.`obs_datetime`, o.`value_coded`, ConceptName(o.`value_coded`) PregStatus FROM `obs` o 
